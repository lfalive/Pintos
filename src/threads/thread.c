...
thread_yield (void) 
{
  ...
  ASSERT (!intr_context ()); //软中断

  old_level = intr_disable ();
  if (cur != idle_thread) //当前线程不是空闲线程
    // list_push_back (&ready_list, &cur->elem); //扔进就绪队列
    list_insert_ordered (&ready_list, &cur->elem, (list_less_func *) &thread_cmp_priority, NULL);
  cur->status = THREAD_READY; //修改进程状态
  schedule ();
  intr_set_level (old_level);
}
...
thread_set_priority (int new_priority)
{
  ...
  cur->base_priority = new_priority; //设置base

  // 如果没有持有锁或者新优先级更高，设置当前优先级然后重新schedule
...
schedule (void) 
{
  ...
  thread_schedule_tail (prev); //获取当前线程，分配恢复之前执行的状态和现场，如果当前线程死了就清空资源
}
